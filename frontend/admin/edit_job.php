...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE jobs SET title = ?, description = ?, location = ?, salary = ?, required_skills = ? WHERE job_id = ?");
    $stmt->bind_param('sssssi', $title, $description, $location, $salary, $required_skills, $job_id);
    $stmt->execute();
    $stmt->close();
...
<head>
    ...
    <title>Edit Job | RecruitFlow</title>
        ...
        <div class="flex-1 p-margin-desktop space-y-xl pt-16">
            <div class="space-y-lg">
                <div class="flex justify-between items-center">
                    <div>
                        <h2 class="font-headline-lg text-headline-lg text-on-surface">Edit Job Posting</h2>
                        <p class="text-on-surface-variant font-body-md text-body-md">Update job posting details and requirements.</p>
                    </div>
            ...
            <form method="post" class="bg-surface-container-lowest p-lg rounded-xl border border-outline-variant/30 shadow-sm max-w-3xl space-y-lg">
                <div>
                    <label class="block text-on-surface-variant font-label-md text-label-md uppercase tracking-wider mb-2">Job Title</label>
                    <input type="text" name="title" value="<?php echo htmlspecialchars($job['title']); ?>" required class="w-full px-4 py-2 border border-outline-variant rounded-lg text-body-md focus:ring-2 focus:ring-primary">
                </div>
                <div class="grid grid-cols-2 gap-lg">
                    <div>
                        <label class="block text-on-surface-variant font-label-md text-label-md uppercase tracking-wider mb-2">Location</label>
                        <input type="text" name="location" value="<?php echo htmlspecialchars($job['location']); ?>" class="w-full px-4 py-2 border border-outline-variant rounded-lg text-body-md focus:ring-2 focus:ring-primary">
                    </div>
                    <div>
                        <label class="block text-on-surface-variant font-label-md text-label-md uppercase tracking-wider mb-2">Salary</label>
                        <input type="text" name="salary" value="<?php echo htmlspecialchars($job['salary']); ?>" class="w-full px-4 py-2 border border-outline-variant rounded-lg text-body-md focus:ring-2 focus:ring-primary">
                    </div>
                </div>
                <div>
                    <label class="block text-on-surface-variant font-label-md text-label-md uppercase tracking-wider mb-2">Required Skills</label>
                    <input type="text" name="required_skills" value="<?php echo htmlspecialchars($job['required_skills']); ?>" class="w-full px-4 py-2 border border-outline-variant rounded-lg text-body-md focus:ring-2 focus:ring-primary">
                </div>
                <div>
                    <label class="block text-on-surface-variant font-label-md text-label-md uppercase tracking-wider mb-2">Description</label>
                    <textarea name="description" rows="8" class="w-full px-4 py-2 border border-outline-variant rounded-lg text-body-md focus:ring-2 focus:ring-primary"><?php echo htmlspecialchars($job['description']); ?></textarea>
                </div>
