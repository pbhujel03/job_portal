...
$stmt = $conn->prepare("SELECT user_id, full_name, email, role, phone, profile_image, created_at FROM users WHERE user_id = ?");
...
$role_display = $user['role'] === 'job_seeker' ? 'Candidate' : ($user['role'] === 'recruiter' ? 'Recruiter' : 'Admin');
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>View User | Job Portal Admin</title>
        ...
        <div class="flex-1 p-margin-desktop space-y-xl pt-16">
            <div class="space-y-lg">
                <div class="flex justify-between items-center">
                    <div>
                        <h2 class="font-headline-lg text-headline-lg text-on-surface">User Details</h2>
                        <p class="text-on-surface-variant font-body-md text-body-md">View user information and profile details.</p>
                    </div>
                    <div class="flex gap-2">
                        <a href="edit_user.php?id=<?php echo $user['user_id']; ?>" class="bg-primary hover:bg-primary-container text-white px-6 py-3 rounded-lg font-bold flex items-center transition-all shadow-sm">
                            <span class="material-symbols-outlined mr-2">edit</span>Edit User
                        </a>
            ...
            <div class="bg-surface-container-lowest p-lg rounded-xl border border-outline-variant/30 shadow-sm max-w-2xl">
                <div class="grid grid-cols-2 gap-lg">
                    <div>
                        <p class="text-on-surface-variant font-label-md text-label-md uppercase tracking-wider">Full Name</p>
                        <p class="font-body-lg text-on-surface mt-2"><?php echo htmlspecialchars($user['full_name']); ?></p>
                    </div>
                    <div>
                        <p class="text-on-surface-variant font-label-md text-label-md uppercase tracking-wider">Email</p>
                        <p class="font-body-lg text-on-surface mt-2"><?php echo htmlspecialchars($user['email']); ?></p>
                    </div>
                    <div>
                        <p class="text-on-surface-variant font-label-md text-label-md uppercase tracking-wider">Role</p>
                        <div class="mt-2">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium <?php echo ($user['role'] === 'job_seeker') ? 'bg-blue-100 text-blue-800' : (($user['role'] === 'recruiter') ? 'bg-purple-100 text-purple-800' : 'bg-red-100 text-red-800'); ?>">
                                <?php echo $role_display; ?>
                            </span>
                        </div>
                    </div>
                    <div>
                        <p class="text-on-surface-variant font-label-md text-label-md uppercase tracking-wider">Phone</p>
                        <p class="font-body-lg text-on-surface mt-2"><?php echo htmlspecialchars($user['phone'] ?? 'Not provided'); ?></p>
                    </div>
                    <div class="col-span-2">
                        <p class="text-on-surface-variant font-label-md text-label-md uppercase tracking-wider">Member Since</p>
                        <p class="font-body-lg text-on-surface mt-2"><?php echo date('M d, Y', strtotime($user['created_at'])); ?></p>
                    </div>
                </div>
            </div>
        </div>
